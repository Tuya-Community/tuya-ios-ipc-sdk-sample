...
- (IBAction)sendVerificationCode:(UIButton *)sender {
    if ([self.emailAddressTextField.text containsString:@"@"]) {
        ...
        NSString *region = [[TuyaSmartUser sharedInstance] getDefaultRegionWithCountryCode:self.countryCodeTextField.text];
        [[TuyaSmartUser sharedInstance] sendVerifyCodeWithUserName:self.emailAddressTextField.text region:region countryCode:self.countryCodeTextField.text type:3 success:^{
