...
@interface CameraCloudViewController ()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource,TuyaTimelineViewDelegate,TuyaSmartCloudManagerDelegate>
...
@implementation CameraCloudViewController
...
- (void)viewDidLoad {
    ...
    self.timeLineView = [[TuyaTimelineView alloc] initWithFrame:CGRectMake(0, self.dayCollectionView.bottom, [UIScreen mainScreen].bounds.size.width, 74)];
...
- (void)soundAction {
    ...
    [self.cloudManager enableMute:!isMuted success:^{
        ...
        [self alertWithMessage:NSLocalizedStringFromTable(@"enable mute failed", @"IPCLocalizable", @"")];
    }];
}
        ...
        if (result) {
            if (self.isRecording) {
                ...
                if ([self.cloudManager stopRecord] != 0) {
                    [self alertWithMessage:NSLocalizedStringFromTable(@"record failed", @"IPCLocalizable", @"")];
                }else {
                    [self alertWithMessage:NSLocalizedStringFromTable(@"ipc_multi_view_video_saved", @"IPCLocalizable", @"")];
                }
            }else {
...
- (void)pauseAction {
    if (self.isPlaying && self.isPaused) {
        if ([self.cloudManager resumePlayCloudVideo]) {
            [self alertWithMessage:NSLocalizedStringFromTable(@"fail", @"IPCLocalizable", @"")];
        }else {
    ...
    [self checkPhotoPermision:^(BOOL result) {
        if (result) {
            if ([self.cloudManager snapShoot]) {
                [self alertWithMessage:NSLocalizedStringFromTable(@"ipc_multi_view_photo_saved", @"IPCLocalizable", @"")];
            }else {
                [self alertWithMessage:NSLocalizedStringFromTable(@"fail", @"IPCLocalizable", @"")];
            }
        }
...
- (void)checkCloudState:(TuyaSmartCloudState)state {
    switch (state) {
        case TuyaSmartCloudStateNoService:
            ...
            [self alertWithMessage:NSLocalizedStringFromTable(@"ipc_cloudstorage_status_off", @"IPCLocalizable", @"")];
            break;
        case TuyaSmartCloudStateNoData:
        case TuyaSmartCloudStateExpiredNoData:
            [self alertWithMessage:NSLocalizedStringFromTable(@"ipc_cloudstorage_noDataTips", @"IPCLocalizable", @"")];
            break;
        case TuyaSmartCloudStateLoadFailed:
            [self alertWithMessage:NSLocalizedStringFromTable(@"ty_network_error", @"IPCLocalizable", @"")];
            break;
...
- (void)loadTimePieceForDay:(TuyaSmartCloudDayModel *)dayModel {
    ...
    [self.cloudManager timeLineWithCloudDay:dayModel success:^(NSArray<TuyaSmartCloudTimePieceModel *> *timePieces) {
        ...
        [self alertWithMessage:NSLocalizedStringFromTable(@"ipc_errormsg_data_load_failed", @"IPCLocalizable", @"")];
    }];
...
- (void)playCloudTimePiece:(TuyaSmartCloudTimePieceModel *)pieceModel playTime:(NSInteger)playTime {
    ...
    [self.cloudManager playCloudVideoWithStartTime:playTime endTime:self.selectedDay.endTime isEvent:NO onResponse:^(int errCode) {
        ...
        [weakSelf alertWithMessage:NSLocalizedStringFromTable(@"ipc_video_end", @"IPCLocalizable", @"")];
    }];
}
...
- (void)checkPhotoPermision:(void(^)(BOOL result))complete {
    if ([CameraPermissionUtil isPhotoLibraryNotDetermined]) {
        ...
        [self alertWithMessage:NSLocalizedStringFromTable(@"Photo library permission denied", @"IPCLocalizable", @"")];
